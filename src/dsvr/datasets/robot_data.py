...
@dataclass
class RoboticsDatasetV3:
    """
    Robotics dataset stored in a single ``.npz`` file (v1.3).

    Extends RoboticsDatasetV2 by adding segmentation masks with the same dimensions
    as depth images.

    Streams: RGB images, depth images, **segmentation masks**, joint states,
    **static SE(3) frames**, and **per-name SE(3) trajectories**, plus optional
    force/torque (F/T). Each stream has its own timestamps.

    Static SE(3): same as before (v1.1)
      - ``se3_names`` : (K,)
      - ``se3_mats``  : (K,4,4)

    SE(3) trajectories stored PER NAME (no ragged concatenation)
      - In memory:  ``se3_traj`` is a dict: name -> (times(T,), mats(T,4,4))
      - On disk: two arrays per name:
          ``se3_traj/<encoded_name>/times`` and ``se3_traj/<encoded_name>/mats``
        (name is percent-encoded; decoded on load)

    NEW (v1.3): Segmentation masks
      - ``seg_mask`` : (T_seg, H, W), ``seg_mask_ts`` : (T_seg,)
    ...
    seg_mask: Optional[np.ndarray] = None         # (T_seg, H, W)
    seg_mask_ts: Optional[np.ndarray] = None      # (T_seg,)
    ...
    # --- Per-name SE(3) trajectories (in memory as a dict) ---
    # name -> (times(T,), mats(T,4,4))
    se3_traj: Dict[str, Tuple[np.ndarray, np.ndarray]] = field(default_factory=dict)
    ...
    # --- internal/schema ---
    version: str = field(default=DATASET_VERSION_V3, init=False)
    ...
    @staticmethod
    def _ensure_seg_mask_2d(s: np.ndarray) -> np.ndarray:
        s = np.asarray(s)
        if s.ndim == 3 and s.shape[-1] == 1:
            s = s[..., 0]
        if s.ndim != 2:
            raise ValueError(f"Segmentation mask must be (H,W) or (H,W,1), got {s.shape}")
        return s

    @staticmethod
    def _ensure_seg_masks_3d(S: np.ndarray) -> np.ndarray:
        S = np.asarray(S)
        if S.ndim == 2:
            S = S[None, ...]
        elif S.ndim == 4 and S.shape[-1] == 1:
            S = S[..., 0]
        if S.ndim != 3:
            raise ValueError(f"Segmentation masks must be (N,H,W) or (N,H,W,1), got {S.shape}")
        return S
    ...
    def validate(self) -> None:
        ...
        # Normalize timestamp dtypes
        for ts_name in ["image_ts", "depth_ts", "seg_mask_ts", "joint_ts", "ft_ts"]:
            ts = getattr(self, ts_name)
            if ts is not None:
                setattr(self, ts_name, np.asarray(ts, dtype=np.float64))
        ...
        if self.depth is not None and self.depth.ndim != 3:
            raise ValueError(f"depth must be (T,H,W), got {self.depth.shape}")
        if self.seg_mask is not None and self.seg_mask.ndim != 3:
            raise ValueError(f"seg_mask must be (T,H,W), got {self.seg_mask.shape}")
        if self.joint_states is not None and self.joint_states.ndim != 2:
            raise ValueError(f"joint_states must be (T,DoF), got {self.joint_states.shape}")

        _check_stream(self.images, self.image_ts, "images", "image_ts", lead_dim=4)
        _check_stream(self.depth, self.depth_ts, "depth", "depth_ts", lead_dim=3)
        _check_stream(self.seg_mask, self.seg_mask_ts, "seg_mask", "seg_mask_ts", lead_dim=3)
        _check_stream(self.joint_states, self.joint_ts, "joint_states", "joint_ts", lead_dim=2)
    ...
    def save(self, path: str, compressed: bool = True) -> None:
        ...
        if self.depth_ts is not None: payload["depth_ts"] = self.depth_ts
        # segmentation masks
        if self.seg_mask is not None: payload["seg_mask"] = self.seg_mask
        if self.seg_mask_ts is not None: payload["seg_mask_ts"] = self.seg_mask_ts
        # joints
        if self.joint_states is not None: payload["joint_states"] = self.joint_states
        if self.joint_ts is not None: payload["joint_ts"] = self.joint_ts
        # static SE3
    ...
    @classmethod
    def load(cls, path: str) -> "RoboticsDatasetV3":
        with np.load(path, allow_pickle=False) as npz:
            files = set(npz.files)
            get = lambda k, default=None: npz[k] if k in files else default
            ...
            ds = cls(
                ...
                depth=get("depth"),
                depth_ts=get("depth_ts"),
                seg_mask=get("seg_mask"),
                seg_mask_ts=get("seg_mask_ts"),
                joint_states=get("joint_states"),
                joint_ts=get("joint_ts"),
                se3_names=get("se3_names"),
                se3_mats=get("se3_mats"),
    ...
    def append_seg_mask(self, seg_mask_img: np.ndarray, ts: float | np.ndarray | Iterable[float]) -> None:
        s2 = self._ensure_seg_mask_2d(seg_mask_img)
        ts1 = self._as_float64_ts(ts)
        if ts1.shape[0] != 1:
            raise ValueError("append_seg_mask: expected a single timestamp.")
        if self.seg_mask is not None:
            self._require_same_shape("append_seg_mask (H,W)", tuple(self.seg_mask.shape[1:]), tuple(s2.shape))
        self.seg_mask = self._concat(self.seg_mask, s2[None, ...], axis=0)
        self.seg_mask_ts = self._concat(self.seg_mask_ts, ts1, axis=0)
    ...
    def append_seg_masks(self, seg_masks: np.ndarray, ts: np.ndarray | Iterable[float]) -> None:
        S3 = self._ensure_seg_masks_3d(seg_masks)
        ts = self._as_float64_ts(ts)
        if ts.ndim != 1 or ts.shape[0] != S3.shape[0]:
            raise ValueError(f"append_seg_masks: ts must be 1D of length N={S3.shape[0]}, got {ts.shape}")
        if self.seg_mask is not None:
            self._require_same_shape("append_seg_masks (H,W)", tuple(self.seg_mask.shape[1:]), tuple(S3.shape[1:]))
        self.seg_mask = self._concat(self.seg_mask, S3, axis=0)
        self.seg_mask_ts = self._concat(self.seg_mask_ts, ts, axis=0)
    ...
    def summary(self) -> Dict[str, Tuple]:
        ...
        return {
            ...
            "depth": None if self.depth is None else self.depth.shape,
            "depth_ts": None if self.depth_ts is None else self.depth_ts.shape,
            "seg_mask": None if self.seg_mask is None else self.seg_mask.shape,
            "seg_mask_ts": None if self.seg_mask_ts is None else self.seg_mask_ts.shape,
            "joint_states": None if self.joint_states is None else self.joint_states.shape,
            "joint_ts": None if self.joint_ts is None else self.joint_ts.shape,
            "se3_names": None if self.se3_names is None else self.se3_names.shape,
            "se3_mats": None if self.se3_mats is None else self.se3_mats.shape,
