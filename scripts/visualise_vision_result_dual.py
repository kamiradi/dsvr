...
ap.add_argument("result_npz_a", help="Path to first result .npz saved by Result")
ap.add_argument("result_npz_b", help="Path to second result .npz saved by Result")
ap.add_argument(
    "--urdf",
    type=str,
...
# Load dataset and both results
res_a = VisionInferenceResultV3.load(args.result_npz_a)
res_b = VisionInferenceResultV3.load(args.result_npz_b)
...
# Compute time offset: second result starts right after the first ends
last_time = times[frame_range[-1]]
first_time = times[frame_range[0]]
time_offset = last_time - first_time + (times[frame_range[1]] - first_time)  # gap of one step

count = 0
grand_total = total_frames * 2

for run_idx, res in enumerate([res_a, res_b]):
    run_label = "A" if run_idx == 0 else "B"
    offset = 0.0 if run_idx == 0 else time_offset
    print(f"=== Logging result {run_label} ===")

    for k in frame_range:
        d = times[k]
        t_log = float(d) + offset
        # Set time FIRST before logging any data for this frame
        rr.set_time("frame", duration=t_log)
        X_Cam = mats[k]
        p_WCam_W = X_Cam[:3, 3]
        R_WCam = X_Cam[:3, :3]
        ...
        count+=1
        print(f"\rProcessing {run_label} frame {count}/{grand_total}...", end="", flush=True)
    print()
...
print(f"\rDone! Processed {count} frames ({total_frames} for A + {total_frames} for B).")
